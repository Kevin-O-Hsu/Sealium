...
class ServerConfig:
    ...
    DATABASE_PATH: Path = Path(
        os.environ.get("DATABASE_PATH", PROJECT_ROOT / "data" / "database.db")
    )
    ...
    SERVER_PUBLIC_KEY_PATH: Optional[Path] = (
        ...
        if os.environ.get("SERVER_PUBLIC_KEY_PATH")
        else None
    )

    # ==================== 安全配置 ====================
    # 时间戳允许偏差（秒）
    TIME_STAMP_TOLERANCE_SECONDS: int = int(
        os.environ.get("TIME_STAMP_TOLERANCE_SECONDS", 300)
    )
