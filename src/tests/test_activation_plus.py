...
class TestBoundaryConditions:
    ...
    def test_timestamp_boundary(
        self, activator, storage, clean_unused_activation_code, monkeypatch
    ):
        """时间戳边界测试 - 验证300秒容忍度"""
        ...
        for delta in [-tolerance, tolerance]:
            ...
            monkeypatch.setattr(
                Utils, "get_timestamp_from_api", lambda timeout=None: mock_timestamp
            )
    ...
    def test_machine_code_consistency(
        self, activator, storage, clean_unused_activation_code
    ):
        """多次激活同一机器，机器码应一致"""
...
class TestExceptionScenarios:
    ...
    def test_server_returns_empty_response(
        self, activator, clean_unused_activation_code, monkeypatch
    ):
        def mock_post(*args, **kwargs):
            resp = requests.Response()
...
class TestConcurrency:
    ...
    def test_concurrent_activation_same_code(
        self, activator, storage, clean_unused_activation_code
    ):
        ...
        assert (
            success_count == 10
        ), f"Expected 10 successes, got {success_count}, results: {results[:5]}"
    ...
    def test_concurrent_activation_different_machine(self, activator, storage):
        ...
        def worker(machine_id):
            ...
            try:
                with patch(
                    "sealium.common.utils.Utils.generate_machine_code",
    ...
    def test_concurrent_generate_activation_codes(self, storage):
        ...
        codes_in_db = [
            c.activation_code for c in all_codes if c.activation_code in codes_list
        ]
...
class TestSecurity:
    ...
    def test_replay_attack_with_different_nonce(
        self, activator, storage, clean_unused_activation_code
    ):
        """同一机器重复激活应成功"""
        test_code = clean_unused_activation_code
    ...
    def test_replay_attack_different_machine(
        self, activator, storage, used_activation_code_different_machine
    ):
        """已用于其他机器的激活码，无法激活"""
        ...
        assert response.result == "error"
        assert (
            "已被使用" in response.error_msg or "已被其他设备使用" in response.error_msg
        )
...
class TestDataConsistency:
    ...
    def test_activation_twice_different_machine(
        self, activator, storage, clean_unused_activation_code
    ):
        ...
        with patch(
            "sealium.common.utils.Utils.generate_machine_code",
            side_effect=mock_machine_code,
        ):
            resp2 = activator.activate(test_code)
            assert resp2.result == "error"
            assert (
                "已被使用" in resp2.error_msg or "已被其他设备使用" in resp2.error_msg
            )
    ...
    def test_activation_code_uniqueness(self, storage):
        ...
        activation = ActivationCode(
            activation_code=code, status=ActivationStatus.UNUSED
        )
...
class TestSameMachineReactivation:
    def test_same_machine_reactivation_success(
        self, activator, used_activation_code_same_machine
    ):
        response = activator.activate(used_activation_code_same_machine)
        assert response.result == "success"
        assert response.authorized_until is not None

    def test_different_machine_activation_fails(
        self, activator, used_activation_code_different_machine
    ):
