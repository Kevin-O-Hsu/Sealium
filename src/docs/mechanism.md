...
- 服务端生成 RSA 密钥对（4096 位）：
  - 私钥 `server_private.pem`：服务端持有，用于解密客户端请求
  - 公钥 `server_public.pem`：分发给客户端，用于加密请求
- 客户端生成 RSA 密钥对（4096 位）：
  - 私钥 `client_private.pem`：客户端持有，用于解密服务端响应
  - 公钥 `client_public.pem`：服务端持有，用于加密响应
- **密钥分发**：
  - 服务端部署：`server_private.pem` + `client_public.pem`
  - 客户端部署：`server_public.pem` + `client_private.pem`
...
4. 构造明文请求体：
   ...
   ```
5. 使用服务端公钥对明文进行 RSA-OAEP（SHA256）加密，得到密文 `EncReq`
6. 通过 HTTPS 将 `EncReq` 发送至服务端激活接口 `POST /v1/activation`

### 3.2 服务端验证

1. 使用服务端私钥解密 `EncReq`，得到请求明文
2. **时间戳校验**：计算 `|服务器当前时间 - Timestamp|`，若超过配置的容忍度（默认 300 秒），返回错误
3. **防重放检查**：记录已使用的 `(activation_code, nonce)` 组合，若重复则拒绝
4. **激活码校验**：
   - 查询数据库，确认激活码存在
...
6. 构造响应明文：
   ...
   ```
7. 使用客户端公钥对响应明文进行 RSA-OAEP（SHA256）加密，得到密文 `EncResp`
8. 通过 HTTPS 返回 `EncResp`，Content-Type 为 `application/octet-stream`

### 3.3 客户端处理响应

1. 使用客户端私钥解密 `EncResp`，得到响应明文
2. 解析 JSON 为 `ActivationResponse` 对象
3. 若 `result == "success"`，验证 `nonce` 字段存在且非空
...
| 安全措施 | 实现方式 |
|---------|---------|
| **通信加密** | RSA-4096 + OAEP-SHA256，双向加密 |
...
| 错误场景 | 服务端返回 | 客户端行为 |
|---------|-----------|-----------|
| 请求数据为空 | `{"result":"error","error_msg":"请求数据为空"}` | 提示用户 |
| 解密失败 | `{"result":"error","error_msg":"解密失败"}` | 提示错误 |
...
- **代码虚拟化**：客户端关键逻辑（机器码生成、密钥存储、加解密）使用虚拟化保护
...
| 服务端私钥路径 | `SERVER_PRIVATE_KEY_PATH` | `./certs/server_private.pem` |
| 客户端公钥路径 | `CLIENT_PUBLIC_KEY_PATH` | `./certs/client_public.pem`  |
...
activator = Activator(
    ...
    client_private_key_pem=client_priv_key
)
...
```bash
# 1. 生成密钥对
openssl genrsa -out certs/server_private.pem 4096
openssl rsa -in certs/server_private.pem -pubout -out certs/server_public.pem
openssl genrsa -out certs/client_private.pem 4096
openssl rsa -in certs/client_private.pem -pubout -out certs/client_public.pem
